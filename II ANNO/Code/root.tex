...
\item[\ttt{TMath::Gaus(<x>,<mean>,<sigma>,<Bool\_norm>)}] boolean for normalization, default false. If true value divided by $\sqrt{2\pi}\sigma$
\item[\ttt{TMath::Factorial(<int>)}] returns double
\item[\ttt{TMath::Erf(<x>)}] error function \footnote{\[Erf(x) = \frac{2}{\sqrt{\pi}} \int_0^x e^{-t^2} \dd[•]{t}\]}
...
\item[\ttt{Integer(<int\_max>)}] uniform \textbf{integer} distribution on \texttt{\big[0, int\_max - 1\big]}
