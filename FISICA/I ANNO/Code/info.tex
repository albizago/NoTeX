...
\item \textbf{Valutare tempo di esecuzione} \boxed{\texttt{time <exe-path>}} restituisce l'output del programma e tre valori:
\begin{verbatim}
real <value1>
user <value2>
sys <value3>
...
\\\boxed{\texttt{git log --oneline --graph}} mostra albero modifiche
...
\\\textbf{In modo compatto} \boxed{\texttt{git checkout -b <branch-name>}} (esegue entrambe le azioni)
\item[Merging] \boxed{1} prima fare checkout nel branch di destinazione (e.g. \texttt{main})
\\\boxed{2} poi \boxed{\texttt{git merge <branch-name>}} per prevenire che, in caso di cambiamenti lineari (nessuna concorrenza), Git 'dimentichi' il branch da mergeare e trasferisca direttamente tutti i commit nel branch di arrivo si può aggiungere \boxed{\texttt{--no-ff}} \textbf{(utilizzato di rado)}
...
\item Posizionatisi nel workspace, \boxed{\texttt{ssh-keygen -t rsa}} per creare chiave. \textbf{Annotare il percorso del file \texttt{.pub}}
...
\item Nel workspace \boxed{\texttt{git remote add origin <repository_ssh_url>}} per collegare l repo locale a remoto
\item \boxed{\texttt{git push -u origin main}} per sincronizzare repo tramite il \texttt{push} e impostare lo stream remoto di default
...
\item[Aggiornare remote repo] \boxed{\texttt{git push}} pusha commit \textbf{dal local repo} (tutto quello che non è stato committato non viene caricato)
\\Per branch \boxed{\texttt{git push <remote-branch> <local-branch>}}
\end{description}
Il \texttt{push} fallisce se local non è in pari con il remote: possibile recuperare dal remote \textbf{senza merge con il local} con \boxed{\texttt{git fetch}}
...
\\Per \textbf{clonare e condividere repository} \boxed{\texttt{git clone <remote_url>}} per scaricare in locale contenuto dal remoto. Cloning e pulling possibili per repo pubblici, \textbf{pushing solo per collaboratori}
