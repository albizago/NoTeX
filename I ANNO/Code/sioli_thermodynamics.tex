...
Si introduce la
\lawbox{I Equazione dell'energia}{\bigg(\pdv{U}{V}\bigg)_\theta = \theta \bigg(\pdv{p}{\theta}\bigg)_V - p}
...
\lawbox{U per GR}{U(\theta, V) = n \molhtv \theta - \frac{a n^2}{V} + cost}
...
\\~\\Per $C_1 \gg C_2$, ovvero $\ds r \equiv \frac{C_2}{C_1} \rightarrow 0$ il corpo $1$ si comporta come termostato
...
\[\Delta S_1 = C_1 \ln \bigg( \frac{1 + rR}{1+r} \bigg) \approx C_1\frac{1+r}{1+Rr} \bigg(\frac{R (1+r) - (1+rR)}{(1+r)^2}\bigg)\bigg|_{r=0}^{•}\cdot r = C_1\frac{(R-1)}{(1+r) (1+Rr)}\bigg|_{r=0}^{•}\cdot r = C_1 \cdot r(R-1) =\]
...
\[\bigg(\pdv{U}{V}\bigg)_{T, \vec{n}} = - p + T \bigg(\pdv{S}{V}\bigg)_{T, \vec{n}}\]
in quanto $\ds \dd[•]{S} = \bigg(\pdv{S}{V}\bigg)_T \dd{V}$ per $\dd[•]{T} = 0$.\\
Applicando quindi la seconda relazione di Maxwell si ha
\[\bigg(\pdv{S}{V}\bigg)_{T, \vec{n}} = \bigg(\pdv{p}{T}\bigg)_{V, \vec{n}}\]
...
Ciò che essa determina è dunque che la trattazione statistica sull'\textit{ensemble statistico} (e dunque i valori medi che se ne ricavano) è equivalente a quella sull'evoluzione temporale: mediare nel tempo equivale a mediare nell'ensemble.

\begin{description}
\item[Ensemble statistico] un'idealizzazione consistente in un numero pressoché infinito di copie virtuali di un sistema fisico che ne descrivono i possibili stati, considerati contemporaneamente.
\end{description}
